...
static VALUE c_stop(VALUE self)
{
  /* CSOUND *csnd; */
  userData *ud;
  Data_Get_Struct(self, userData, ud);
...
static VALUE c_performKsmps(VALUE self)
{
  userData *ud;
  Data_Get_Struct(self, userData, ud);
  csoundPerformKsmps(ud->csound);
  return Qnil;
}
...
{ 
  ...
  if (!udata->result) {
    ...
    printf("PERF_STATUS: %d\n", udata->PERF_STATUS);
    printf("perform buffer %d\n", csoundPerformBuffer(udata->csound));
    while ((csoundPerformKsmps(udata->csound) == 0) &&
           (udata->PERF_STATUS == 1)) {
      printf("PERF_STATUS: %d\n", udata->PERF_STATUS);
      if (udata->PERF_STATUS == 0) break;
    }
    printf("PERF_STATUS: %d\n", udata->PERF_STATUS);
...
void Init_ruby_csound() {
  ...
  rb_define_method(cCsound, "performKsmps", c_performKsmps, 0);
  rb_define_method(cCsound, "performThread", c_performThread, 0);
  rb_define_method(cCsound, "end", c_end, 0);
  rb_define_method(cCsound, "destroy", c_destroy, 0);
  rb_define_method(cCsound, "stop", c_stop, 0);
